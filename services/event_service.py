...
def add_new_event(id:int, dia:int, hora:int, mes:int, anyo:int, descripcio:str, db:Session):
    db_event = Event(id= id, dia=dia, hora=hora, mes=mes, anyo=anyo, descripcio=descripcio)
    db.add(db_event)
    db.commit()
    db.refresh(db_event)
    return {"msg":"Created event succesfully"}

def update_event(id: int, dia:int, hora:int, mes:int, anyo:int, descripcio:str,db:Session):
